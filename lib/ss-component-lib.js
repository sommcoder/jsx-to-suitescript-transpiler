  ...
  Search: {
    add: (props, currNode) => {
      ...
      /*
        ...
        WE DON'T HAVE THE CHILD PROPS HANDLED YET!


        SHOULD WE CALL THIS FUNCTION AFTER WE TRAVERSE THE WHOLE TREE? THE BENEFIT WOULD BE THAT WE WOULD ACTUALLY HAVE THE COMPTYPE OF THE SEARCH COMPONENTS CHILDREN AND THEREFORE BE ABLE TO CALL THIS ADD() FUNCTION WITH THAT INFORMATION NOW AVAILABLE. BUT WE CAN'T SEEM TO DO IT EFFICIENTLY DURING AST TRAVERSAL!
         
        */
      console.log("currNode.children:", currNode.children);
      console.log(
        "currNode.children.every((child) => child.compType === 'Field'):",
        currNode.children.every((child) => child.compType === "Field")
      );
      // if every child is a field
      if (currNode.children.every((child) => child.compType === "Field")) {
        currNode.children.map((child, i, arr) => {
          searchCompSyntax += `${props.parentVar}.setSublistValue({\n   id: ${
          ...
          // if child has children and are ALL Select components, map through them
          if (
            child.children.length > 0 &&
            child.children.every((child) => child.compType === "Select")
          ) {
            child.children.map((kid) => {
              searchCompSyntax += `${props.pageVar}.addSelectOptions({\n    text: result.getText({\n    name: ${kid.props.arguments.text},\n}),\n   value: result.getValue({\n    name: ${kid.props.arguments.value},\n}),\n`;
            });
          }
        });
      }

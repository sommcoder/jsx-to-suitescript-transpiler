    ...
    function jsxToSS() {
      ...
      function handleNode(path) {
        ...
        path.node.children = path.node.children.filter(
          (child) =>
            child.type !== "JSXText" &&
            child.openingElement.name.name !== "Search"
          // Search cannot be a child/parent
        );
        ...
        if (parentPath.node.compType === "Search") {
      ...
      function uniformPropTypes(compObj, compType, typeProp, parentType) {
      ...
      // if search component, get children and their column prop values
      //
      ...
      function handleProps(compType, propsArr, path) {
        // props need to be handled depending on their type, binding and whether they're wrapped in a JSX Expression and combinations thereof
        let propsObj = {};
      ...
      function getSSComponentCalls(currNode) {
        // PAGE COMPONENT:
        if (currNode.compType === "Search") console.log("SEARCH", currNode);

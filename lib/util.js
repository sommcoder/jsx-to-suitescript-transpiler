...
export function createComponentId(label, page, // optional because the component COULD == Page
parent) {
    return `${"custpage_" + label.toLowerCase().split(" ").join("_")}${parent ? "_" + parent : ""}_${page}`;
}
export function getSSComponentCalls(comp, attr, path, children) {
    console.log("comp:", comp);
    console.log("attr:", attr);
    // jsx is NOT included in the SS library: error!
    if (!SS.hasOwnProperty(comp)) {
        throw path.buildCodeFrameError(`ERR: the jsx component: ${comp}, is NOT included in the ss library. Refer to docs to see included components`);
    }
...
export function convertToCamelCase(label, path) {
    ...
    if (label && /^[a-zA-Z0-9_]+$/.test(label))
        ...
        throw path.buildCodeFrameError(`ERROR: jsx label: ${label} contains special characters that are not permitted`);
}
